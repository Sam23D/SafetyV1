...
- (NSTimer *) timer {
    if (!_timer) {
        _timer = [NSTimer timerWithTimeInterval:timerInterval target:self selector:@selector(connectionDidFinishLoading:) userInfo:nil repeats:YES];
    }
    return _timer;
}
...
-(void) showAlert {
    ...
    UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"ALERT" message:@"There is an emergency near your area. Please, head to the nearest safe point." delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil, nil];
    [alert show];
}
...
- (void)viewDidAppear:(BOOL)animated {
    mapView_.padding = UIEdgeInsetsMake(0, 0, 48, 0); //To move by pixels the position of the MyLocationButton
}
